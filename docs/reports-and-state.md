...
Use `repovista reports` to browse completed runs in the terminal, select a run, and navigate the full combined report or each generated section. In the run list, Space marks or unmarks report runs for deletion, and `d` opens a confirmation screen before the marked run directories are removed.
